            ...
            <div class="container-fluid">
                ...
                <div class="py-3">
                    <div class="card shadow mb-4">
                        ...
                        $builder->select("(SELECT MIN(timeline.tglmulai) FROM timeline) AS tglawal", FALSE);
                        $query = $builder->get(1);
                        ...
                        <?php
                        $builder = $db->table('timeline');
                        $builder->select("(SELECT MAX(timeline.tglselesai) FROM timeline) AS tglakhir", FALSE);
                                        ...
                                        <?php foreach ($kategori as $data1) : ?>
                                            ...
                                            <?php foreach ($timeline2 as $data2) : ?>
                                                <?php if ($data1['ktgnoitem'] == $data2['ktgnoitem']) { ?>
                                                    <?php
                                                    ?>
                                                    <!-- Mengambil Data tgl mulai Pekerjaan setiap No Item -->
                                                    <?php
                                                    $uraian = $data2['uraian'];
                                                    $db      = \Config\Database::connect();
                                                    $builder = $db->table('timeline');
                                                    $builder->select("(SELECT (timeline.tglmulai) FROM timeline WHERE timeline.uraian='$uraian') AS tglmulaipek", FALSE);
                                                    $query = $builder->get();
                                                    $d = $query->getResultArray();
                                                    $tglmulaipek = $d[0]['tglmulaipek'];
                                                    ?>
                                                    <!-- Mengambil Data tgl selesai Pekerjaan setiap No Item -->
                                                    <?php
                                                    $builder->select("(SELECT (timeline.tglselesai) FROM timeline WHERE timeline.uraian='$uraian') AS tglselesaipek", FALSE);
                                                    $query2 = $builder->get();
                                                    ...
                                                    $jmlmgpek = ceil($mgpek);
                                                    ?>
                                                    <!-- Nilai bobot pekerjaan per minggu -->
                                                    <?php
                                                    $ktgnoitem = $data1['ktgnoitem'];
                                                    $db      = \Config\Database::connect();
                                                    $builder = $db->table('timeline');
                                                    $builder->select("(SELECT SUM(timeline.jmlharga) FROM timeline) AS totalharga", FALSE);
                                                    $query1 = $builder->get();
                                                    ...
                                                    <tr>
                                                        ...
                                                        <?php
                                                        // cek minggu tglmulaipek
                                                        $tglmulaipek2 = new DateTime($tglmulaipek);
                                                        $jmlharipek = $tglmulaipek2->diff($tgl11)->days + 1;
                                                        $mg2 = $jmlharipek / 7;
                                                        $jmlmg2 = ceil($mg2);
                                                        // echo date('d', strtotime($tglmulaipek));
                                                        // cek minggu tglselesaipek
